    ...
    $.fn.pageList = function (arg, methodName) {
        if (typeof methodName === "string" && $.trim(methodName) != "") {
            ...
            var defaultSettings = {
                ...
                showGoLink: false, //是否显示快速跳转，可能的值为auto 自动判断；true 永远显示；false 永不显示  
                goInputType: 'select', //如何显示快速跳转输入，可能的值为select 下拉列表；text 输入框，默认值为select  
                goText: 'Go', //显示的快速跳转文本  
                recordText: '', //显示记录数，为空时不显示，否则按照占位符显示文本，{0}表示总页数，{1}表示总记录数  
                clickCallback: function clickCallback(currentPage) {}, //链接被点击时触发的事件，currentPage表示当前点击的是第几页，索引从1开始  
            ...
            $(this).each(function () {
                ...
                if (totalPages > 0) {
                    ...
                    var startPage = 2,
                        endPage = totalPages - 1; //排除首页最后一页后显示的第一个超链接，最后一个超链接  
                    var prevS = false,
                        nextS = false; //是否需要显示…对应的超链接  
                    if (arg.showPageRange + 4 < totalPages) {
                        //因为页面链接最多包含arg.showPageRange + 4个，所以如果总页数大于时，才显示对应的…  
                        if (!arg.currentPageCenter) {
                            ...
                            if (startPage >= endPage) {
                                startPage = startPage - arg.showPageRange;
                            } //修正  
                            ...
                            if (endPage - startPage < arg.showPageRange) {
                                endPage = startPage + arg.showPageRange - 1;
                            } //根据startPage修正endPage  
                            if (endPage > totalPages - 1) {
                                endPage = totalPages - 1;startPage = endPage - arg.showPageRange + 1;
                    ...
                    var li = renderDoms('首  页', arg.currentPage == 1, false, 1, arg); //前一页  
                    li.addClass("firstpage");
                    ul.append(li);

                    li = renderDoms(arg.prevText, arg.currentPage == 1, false, arg.currentPage - 1, arg); //前一页  
                    li.addClass("prev");
                    ul.append(li);
                    ...
                    var li = renderDoms('尾  页', arg.currentPage == totalPages, false, totalPages, arg); //前一页  
                    li.addClass("firstpage");
                    ul.append(li);
                    var showGo;
                    ...
                    if (typeof arg.recordText === "string" && $.trim(arg.recordText) != "") {
                        $(this).append('<span class="text">' + arg.recordText.replace(/\{0\}/g, arg.pageSize).replace(/\{1\}/g, arg.totalCount) + '</span>');
                    }
                }
            });
        }
