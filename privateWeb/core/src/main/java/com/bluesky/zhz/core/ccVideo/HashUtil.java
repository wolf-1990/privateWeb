...
public class HashUtil {
	 ...
	 * @return
	 */
	public static String md5(ChunkInfo chunkInfo) {
		try {
			MessageDigest md = MessageDigest.getInstance("MD5");
			FileInputStream fis = chunkInfo.getFileInputStream();
			byte[] buffer = new byte[1024];
			int length = -1;
			while ((length = fis.read(buffer)) != -1) {
				md.update(buffer, 0, length);
	 ...
	 * @param str
	 *            十六进制字符串。
	 * @return 如果str为null，或不是一个合法的偶长度十六进制字符串，则返回null；如果str长度为0，则返回new
	 *         byte[0];否则返回转换后的结果。
	 */
